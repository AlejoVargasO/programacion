...
#camera = cv2.VideoCapture("/dev/video0") # Este es el puerto de la cámara
camera = cv2.VideoCapture('videoprueba.mp4') #utilizamos la ruta del Video para la inferencia.
camera.set(cv2.CAP_PROP_FRAME_HEIGHT,480)
camera.set(cv2.CAP_PROP_FRAME_WIDTH,480)

#Entramos en un bucle para la ejecución de la inferencia
while True:
	r,frame = camera.read()
	...
	for detect in detections:
		...
		if item == "FCar": color = (0,255,0) #Cando se detecte un carro se pintará de verde
		else: color = (0,0,255)
